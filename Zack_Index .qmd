...
Label_df <- Stats_stdz_c3 |>
  filter(Player %in% c("Anthony Davis", "Dwayne Wade"))
...
select_players <- c("Anthony Davis", "Dwyane Wade", "Mo Williams", "Channing Frye", "Richard Jefferson")
...
TRY CLUSTERING WITH TOTAL GAMES PLAYED AND POINTS


```{r}
Other_stdz <- Teammates_stats |>
...
elbow_plot <- tibble(k = 1:10) |>
  mutate(
    # List-column of 10 kmeans objects 
    # (apply `kmeans()` to each value of `k`)
    kmeans_results = purrr::map(k, ~kmeans(Other_stdz, .x, nstart=20)),
    # List-column of "glanced" model summaries for each kmeans object
    # (apply `glance()` to each corresponding result after running `kmeans()`)
    glanced = purrr::map(kmeans_results, glance)) |>
  # Turn `glanced` list-column into regular tibble columns
...
#perform k-means clustering with k = 3
Other_kmeans3 <- Other_stdz |>
  select(GP_z, PTS_z) |>
  kmeans(centers = 3, nstart = 20)

#summarize the kmeans object
Other_kmeans3_summary <- tidy(Other_kmeans3)
Other_kmeans3_summary
...
#augment cluster assignments onto original standardized data
Other_stdz_c3 <- augment(Other_kmeans3, Other_stdz)

#visualize the cluster assignments and centroids
ggplot(Other_stdz_c3, aes(y = PTS_z, x = GP_z)) + 
  geom_point(aes(color = .cluster, shape = .cluster)) +
  scale_x_continuous(breaks = scales::breaks_width(0.5)) +
  scale_y_continuous(breaks = scales::breaks_width(0.5)) +
  #Add centroid labels to plot
  geom_label(data = Other_kmeans3_summary, aes(y = PTS_z, x = GP_z, label = cluster, color = cluster),
             size = 3,
             label.r = unit(0.5, "lines"),
             label.size = 1.5,
             label.padding = unit(0.5, "lines"),
             show.legend = FALSE) +
  labs(y = "Standardized Points per Game (PTS_z)",
       x = "Standardized Total Games Played (GP_z)",
       color = "Cluster",
       shape = "Cluster") +
  theme_classic()
...
TRY CLUSTERING BY ALL FEATURES
TRY ADDING A TABLE OF TEAMMATES FROM THE LAST COUPLE YEARS VS TEAMMATES FROM HIS CHAMPIONSHIP YEARS


```{r}
All_stats <- Teammates_stats |>
